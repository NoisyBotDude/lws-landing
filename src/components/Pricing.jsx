...
const PricingCard = ({ plan, isAnnual }) => {
	...
	return (
		<motion.div
			...
			className={`relative rounded-2xl bg-[#111111] border border-gray-800 p-8 ${plan.popular ? 'bg-gradient-to-b from-[#111111] to-[#1E1435]' : ''
				}`}
		>
			{plan.popular && (
				...
				<div className="text-center mb-16">
					...
					<div className="flex items-center justify-center gap-4 mb-12">
						...
						<button
							...
							className={`relative w-16 h-8 rounded-full transition-colors duration-200 ${isAnnual ? 'bg-[#8B5CF6]' : 'bg-gray-700'
								}`}
						>
							<div
								className={`absolute top-1 left-1 w-6 h-6 rounded-full bg-white transition-transform duration-200 ${isAnnual ? 'translate-x-8' : 'translate-x-0'
									}`}
							/>
						</button>
						<span className={`text-sm ${isAnnual ? 'text-white' : 'text-gray-400'}`}>
